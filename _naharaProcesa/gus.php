...
if($_GET["esframe"]==1) 
{
	$_SESSION["esframe_gus"]=1;
	$_SESSION["esframe_gus_id"]=$_GET["registro"];	
	$resultx = @mysqli_query($GLOBALS["enlaceDB"] ,"select ayudatabla from catablas where idtabla=".$_GET["itabla"]);
    while($rowx = mysqli_fetch_array($resultx)) $_SESSION["esframe_gus_archivo"]=$rowx["ayudatabla"];
...
{
	$_SESSION["esframe_gus"]=0;
	$_SESSION["esframe_gus_id"]=0;
	$_SESSION["esframe_gus_archivo"]="";
}
...
$archivoactual="gus.php";
$idcontrolinterno=generaidcontrol();
if($step=="modify") $_SESSION["id_gus"]=$id;
...
if($operacion=="modify") 
{ 
  if($nivelusuario==0) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=8; 
  if($nivelusuario==1) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=8; 
  if($nivelusuario==2) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=8; 
  if($nivelusuario==3) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=8; 
  if($nivelusuario==4) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=8; 
}
if($operacion=="add") 
{ 
  if($nivelusuario==0) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=7; 
  if($nivelusuario==1) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=7; 
  if($nivelusuario==2) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=7; 
  if($nivelusuario==3) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=7; 
  if($nivelusuario==4) if(isset($_POST["iusuariogus"]) || isset($_POST["iretgus"]) || isset($_POST["fechagus"])) $error=7; 
}
...
<?
if($_SESSION["esframe_gus"]==1)
{
  if($_SESSION["esframe_gus_archivo"]=="usuarios")
  {
    if($step=="add")
    {
      $iusuariogus=$_SESSION["id_usuarios"];
    }
    if($step=="busqueda2" || $step=="busqueda3" || $step=="1")
    {
      $iusuariogusb1="=";
      $iusuariogusb2=$_SESSION["id_usuarios"];
    }
  }
  else   if($_SESSION["esframe_gus_archivo"]=="ret")
  {
    if($step=="add")
    {
      $iretgus=$_SESSION["id_ret"];
    }
    if($step=="busqueda2" || $step=="busqueda3" || $step=="1")
    {
      $iretgusb1="=";
      $iretgusb2=$_SESSION["id_ret"];
    }
  }
...
<title><? echo("Gusta"); if(isset($titulobusqueda)) echo(". ".$titulobusqueda);?></title>
...
<script language="javascript" type="text/javascript" src="include/autocompleta/ajax.js"></script>
<script language="javascript" type="text/javascript" src="include/autocompleta/ajax-dynamic-list.js"></script>
<link href="include/autocompleta/estilos.css" rel="stylesheet" type="text/css"/>
</head>
...
<?php 
  ...
  if($operacion=="modify" || $operacion=="add") 
  {
	...
	if($operacion=="modify" || $operacion=="add") 
	{   
      $sqltemporal.=construyesqltemporal("iusuariogus","",0);
$sqltemporal.=construyesqltemporal("iretgus","",0);
$sqltemporal.=construyesqltemporal("fechagus","'",0);
$sqltemporal.=construyesqltemporal("activo","",0);
	...
	{	
	  $controlmatch="SI";  
	   if($nivelusuario=="meminpinguin") {	
		  ...
		  $sql = "INSERT INTO gus SET " .$sqltemporal;
		  ...
		  if ( @mysqli_query($GLOBALS["enlaceDB"] ,$sql) ) 
		  {
			...
			$idcontrolinterno=generaidcontrol();
			 $sqltemporal= "idusuarioseguimiento=".$sesionid.",idaccesoseguimiento=".$sesionidregistro.",horaseguimiento='".$ultimahoraentrada."',registroseguimiento=".$id.",tablaseguimiento=4,operacionseguimiento='2'"; @mysqli_query($GLOBALS["enlaceDB"] ,"INSERT INTO caseguimiento SET " .$sqltemporal);		
			$_SESSION["id_gus"]=$id;
            if($_GET["edicioninterior"]==1)
            {
            	$_SESSION["frame_interior_gus"]="OK";
	...
	{	  
	  $controlmatch="SI";	
	   if($nivelusuario=="10") {	      
		  $sql = "UPDATE gus SET " .$sqltemporal. " WHERE ID=".$id;
		  if($_SESSION["sesionmododepuracion"]=="SI") echo($sql);
		  if ( @mysqli_query($GLOBALS["enlaceDB"] ,$sql) ) 
		  {
			...
			{  
			  $mensaje.=$ib_add_modify;
			   $sqltemporal= "idusuarioseguimiento=".$sesionid.",idaccesoseguimiento=".$sesionidregistro.",horaseguimiento='".$ultimahoraentrada."',registroseguimiento=".$id.",tablaseguimiento=4,operacionseguimiento='1'"; @mysqli_query($GLOBALS["enlaceDB"] ,"INSERT INTO caseguimiento SET " .$sqltemporal);
			...
			else
			{
			  ...
			  $modomensaje="NADA";
              if($_GET["edicioninterior"]==1)
	              $_SESSION["frame_interior_gus"]="NADA";
			}  
        	if($_GET["edicioninterior"]==1)
            {
				/* echo("<script>parent.location.reload()</script>");
...
$mensajedelete="";if($mensajedelete<>"") { $step="modify"; $operacion=""; $modomensaje="ERROR"; $mensaje.="No se puede eliminar el registro, debido a lo siguiente:".$mensajedelete;
    ...
    {
       if($nivelusuario=="10") {
		$sql = "DELETE FROM gus WHERE id=".$id;
		if($_SESSION["sesionmododepuracion"]=="SI") echo($sql);
		if ( @mysqli_query($GLOBALS["enlaceDB"] ,$sql) ) 
		{
		  $mensaje.=$ib_delete_bien." <a href=\"javascript:window.history.go(-2)
	;\" class=\"boton80\">".$ib_regresar."</a>";
		   $sqltemporal= "idusuarioseguimiento=".$sesionid.",idaccesoseguimiento=".$sesionidregistro.",horaseguimiento='".$ultimahoraentrada."',registroseguimiento=".$id.",tablaseguimiento=4,operacionseguimiento='3'"; @mysqli_query($GLOBALS["enlaceDB"] ,"INSERT INTO caseguimiento SET " .$sqltemporal);
		  
		  $step="busqueda";
		  $operacion="";
          if($_GET["edicioninterior"]==1)
          {
          	$_SESSION["frame_interior_gus"]="BORRADO";
          	echo("<script>parent.location.reload()</script>");
          	exit();
          }     
		} 
...
if($step=="modify"){
$resultx = @mysqli_query($GLOBALS["enlaceDB"] ,"SELECT id,iusuariogus FROM gus where id=". $id);
$rowx = mysqli_fetch_array($resultx);
$linkx=" >> ".$rowx["iusuariogus"]." ".$rowx[""];
$idx=$rowx[""];
}
echo("<a href=gus.php?step=1".$url_extra."><span class=titulo>Gusta</span></a>".$linkx3.$linkx2.$linkx1.$linkx);?><? } else { ?><? if(isset($titulobusqueda)) echo($titulobusqueda." ");?><? } ?></td>
	<td align="left" ><? if($ocultabotones<>1) { ?><? $botones=""; if($nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) $botones.="<td><a href=gus.php?step=busqueda3".$url_extra."><img src=recursos/botonlistar.gif border=\"0\" alt=\"Listar Gusta\"></a></td>";
if($nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) $botones.="<td><a href=gus.php?step=busqueda".$url_extra."><img src=recursos/botonbuscar.gif border=\"0\" alt=\"Buscar Gusta\"></a></td>";
 if($_GET["edicioninterior"]<>1) echo("<table class=\"textogeneral\"><tr><td class=\"textogeneral\" align=\"right\">".$botones);

echo("</tr></table>"); ?><? } else echo("<a href=\"javascript:self.parent.tb_remove();\"><img src=\"recursos/botoncerrar.gif\" border=\"0\"></a>"); ?></td>	
  </tr>
</table>
<? } 

  if($_SESSION["frame_interior_gus"]=="OK")
  {
  	$mensaje="Se guardó correctamente el registro";
    $modomensaje="";
  }
  else if($_SESSION["frame_interior_gus"]=="NADA")
  {
  	$mensaje="No hubo cambios en el registro";
    $modomensaje="NADA";
  }
  else if($_SESSION["frame_interior_gus"]=="BORRADO")
  {
  	$mensaje="Se eliminó correctamente el registro";
    $modomensaje="NADA";
  }
  $_SESSION["frame_interior_gus"]="";
...
$activol1="on";  $comparadorsearch="AND"; $sortfield="gus.activo DESC,iusuariogus ASC"; $ordenamiento="";
$activob1="="; $activob2="1";
$iusuariogusl1="on"; 
$iretgusl1="on"; 
} 

$camposbuscadoslistadosearch="gus.id";
cbusqueda1($activol1,"gus","activo");
cbusqueda1($iusuariogusl1,"usuarios","nombreusuario","0","","");
cbusqueda1($iretgusl1,"ret","nombreret","0","","");
cbusqueda1($fechagusl1,"gus","fechagus");
cbusqueda2($iusuariogusl1,"usuarios","gus","iusuariogus","",0,"id");
cbusqueda2($iretgusl1,"ret","gus","iretgus","",0,"id");
cbusqueda3($iusuariogusb1,$iusuariogusb2,"gus","iusuariogus","","0","","");
cbusqueda3($iretgusb1,$iretgusb2,"gus","iretgus","","0","","");
cbusqueda3($fechagusb1,$fechagusb2,"gus","fechagus","'","0","","");
cbusqueda3($activob1,$activob2,"gus","activo","'","0","","");
...
cbusqueda5($iusuariogusl1,"Usuario",": STR","");
cbusqueda5($iretgusl1,"Reto",": STR","");
cbusqueda5($fechagusl1,"Fecha"," : DATE","");
...
function tigra_row_clck(marked_all, marked_one)
{
  if(marked_one!='')
  {
	    window.location.href='gus.php?step=modify&id='+marked_one+'&'
  }
}
...
cbusqueda4($iusuariogusl1,"usuarios","nombreusuario","0","","");
cbusqueda4($iretgusl1,"ret","nombreret","0","","");
cbusqueda4($fechagusl1,"gus","fechagus","0","","");
...
$pietablasearchtigra="\"\"";
cbusqueda6($iusuariogusl1,$sumatoriaiusuariogus,'');
cbusqueda6($iretgusl1,$sumatoriairetgus,'');
cbusqueda6($fechagusl1,$sumatoriafechagus,'');
$pietablasearchtigra.=",\"\"";

?>
<?php echo("var TABLE_CONTENT = [".$listadodecampossearchtigra2.",[".$pietablasearchtigra."]];"); ?><?=$arreglo_ids?></script>
<? if($num_rows>0) { ?><SCRIPT LANGUAGE="JavaScript"> new TTable(TABLE_CAPT, TABLE_CONTENT, TABLE_LOOK);	</SCRIPT><? } ?></td>
  </tr> 
   
   <tr><form name="form2" id="form2" method="post" action="excel/excelgus.php?step=busqueda2<?=$url_extra?>" enctype="multipart/form-data"><input name=activol1 type="hidden" value=<?=$activol1?> ><input name=activob1 type="hidden" value=<?=$activob1?> ><input name=activob2 type="hidden" value=<?=$activob2?> >
<input name=iusuariogusl1 type="hidden" value="<?=$iusuariogusl1?>" ><input name=iusuariogusb1 type="hidden" value="<?=$iusuariogusb1?>" ><input name=iusuariogusb2 type="hidden" value="<?=$iusuariogusb2?>" >
<input name=iretgusl1 type="hidden" value="<?=$iretgusl1?>" ><input name=iretgusb1 type="hidden" value="<?=$iretgusb1?>" ><input name=iretgusb2 type="hidden" value="<?=$iretgusb2?>" >
<input name=fechagusl1 type="hidden" value="<?=$fechagusl1?>" ><input name=fechagusb1 type="hidden" value="<?=$fechagusb1?>" ><input name=fechagusb2 type="hidden" value="<?=$fechagusb2?>" >
<input name=mostrarhijas type="hidden" value=<?=$mostrarhijas?> ><input name=comparadorsearch type="hidden" value="<?=$comparadorsearch?>" ><input name=sortfield type="hidden" value="<?=$sortfield?>" ><input name=ordenamiento type="hidden" value="<?=$ordenamiento?>" ><td class=titulointerior bgcolor="#ffffff" align=right><div align=right><? if($nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) {?><? if($num_rows>0) { ?><input class="textogeneral" type="button" value="Exportar a Excel" name=button2 onClick="return BusquedaExcel('excel/excelgus.php?step=busqueda2');"><? } ?><?} ?><? if($nivelusuario=="meminpinguin") {?><input class="textogeneral" type="button" value="Mensaje masivo" name=button2 onclick="toggle('maquinamensajes')"><?} ?></div></td></form></tr>
   <? } ?>
</table>
...
function wrap (s_, b_ques) {
	...
	<?
if(isset($_GET["iusuariogus"])) $iusuariogus=$_GET["iusuariogus"];
if(isset($_GET["iretgus"])) $iretgus=$_GET["iretgus"];
...
{
if(isset($_POST["iusuariogus"])) $iusuariogus=$_POST["iusuariogus"];
if(isset($_POST["iretgus"])) $iretgus=$_POST["iretgus"];
if(isset($_POST["fechagus"])) $fechagus=$_POST["fechagus"];
}
    if($step=="modify" && $error_unique==0)
	{
	  if($_SESSION["sesionmododepuracion"]=="SI") echo("SELECT * FROM gus where id=". $id);
      $result = @mysqli_query($GLOBALS["enlaceDB"] ,"SELECT * FROM gus where id=". $id);
      if (!$result) 
	  {
...
$id=$row["id"];
$activo=$row["activo"];
$iusuariogus=$row["iusuariogus"];
$iretgus=$row["iretgus"];
$fechagus=$row["fechagus"];
       }
	 }	 
...
?>
      ...
      <form name="form1" id="form1" onSubmit="return enviardatos('N');" method="post" action="gus.php?step=modify&operacion=<?=$step?>&id=<?=$id?>&sortfield=<?=$sortfield?><?=$url_extra?>" enctype="multipart/form-data">
        ...
        <tr>
          <td valign=middle class="titulointerior"><? if($step=="add") echo($ib_agregando); else echo($ib_editando); ?></td>
                    <td><? if($ocultabotones<>1) { ?>					 <div align="right"> <? if($step<>"add") { ?>
				       ...
				       <? if($_GET["edicioninterior"]==1) {  if($nivelusuario=="10") {?><a href="javascript:deleteRecord('gus.php?sortfield=iusuariogus&step=2&operacion=delete&id=<?=$id?>&idcontrol=<?=$idcontrolinterno?>');" class=textoboton>&nbsp;Borrar&nbsp;</a>&nbsp;&nbsp;<?} ?><? } ?>
				          <? } ?>
...
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle" id="t_iusuariogus" name="t_iusuariogus">Usuario * </td><td valign="middle"><? if(($nivelusuario==10)) { ?><select name="iusuariogus" id="iusuariogus"  class=textogeneralform><option value="0" selected></option><?  leecampos("usuarios","id","nombreusuario","",""," "); for ( $i=0; $i<=$registros-1; $i++) { echo("<option value=".$campos1[$i]); if($iusuariogus==$campos1[$i]) { echo(" selected"); } echo(">".$campos2[$i]."</option>"); } ?></select><? } ?><? if(($nivelusuario==10 || $nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2)) { ?><? $valor_mostrar=lee_registro("usuarios","nombreusuario","","",$iusuariogus,"id");
if($valor_mostrar<>"") echo($valor_mostrar); else echo("No encontrado o no aplica");?><? } ?></td></tr><? } ?>
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle" id="t_iretgus" name="t_iretgus">Reto * </td><td valign="middle"><? if(($nivelusuario==10)) { ?><select name="iretgus" id="iretgus"  class=textogeneralform><option value="0" selected></option><?  leecampos("ret","id","nombreret","",""," "); for ( $i=0; $i<=$registros-1; $i++) { echo("<option value=".$campos1[$i]); if($iretgus==$campos1[$i]) { echo(" selected"); } echo(">".$campos2[$i]."</option>"); } ?></select><? } ?><? if(($nivelusuario==10 || $nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2)) { ?><? $valor_mostrar=lee_registro("ret","nombreret","","",$iretgus,"id");
if($valor_mostrar<>"") echo($valor_mostrar); else echo("No encontrado o no aplica");?><? } ?></td></tr><? } ?>
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle" id="t_fechagus" name="t_fechagus">Fecha </td><td valign="middle"><?=$fechagus?></td></tr><? } ?> 
	<? $datostigra=""; ?><? if(($nivelusuario==10)) { ?><? if($datostigra<>"") $datostigra.=","; $datostigra.="'iusuariogus':{'l':'Usuario','r': true,'f':function(n) {return n > 0 && n < 1000000},'t':'t_iusuariogus'}";?><? } ?>
<? if(($nivelusuario==10)) { ?><? if($datostigra<>"") $datostigra.=","; $datostigra.="'iretgus':{'l':'Reto','r': true,'f':function(n) {return n > 0 && n < 1000000},'t':'t_iretgus'}";?><? } ?>
<script>
...
<span class=textogeneral><br></span>
  ...
  <table  border="0" cellspacing="0" cellpadding="0">
    ...
    <tr>
      <td class="spacerlateral"></td>
      <td width=100%  valign=top><form name="form2" method="post" action="gus.php?step=busqueda2&mensajemm=<?=$mensajemm?><?=$url_extra?>" enctype="multipart/form-data"><table width="100%" border="0" cellspacing="1" cellpadding="0" class="bordeprincipal" align="center">
    <tr> 
	  ...
	  <table width="100%" class="titulointerior" cellpadding="0" cellspacing="0">
        <tr>
          <td valign=middle class="titulointerior"><?=$ib_busqueda?></td>
              <td class=textogeneral align="right"><? if($ocultabotones<>1) { ?> <?=$ib_ordenar?><select class="textogeneralform" name=sortfield><option value="iusuariogus" selected>Usuario</option><option value="iretgus">Reto</option><option value="fechagus">Fecha</option></select><select class="textogeneralform" name=ordenamiento><option value=DESC>DESC</OPTION><option value=ASC selected>ASC</OPTION></SELECT>
<input class="textogeneral" type="button" value="<?=$ib_busqueda?>" name=button1 onClick="return BusquedaNormal('gus.php?step=busqueda2&mensajemmx=<?=$mensajemmx?>&boletinelectronicommx=<?=$boletinelectronicommx?><?=$url_extra?>');"><? } ?></td>
</tr>
          </table>
...
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle">Usuario</td><td valign="middle"><? if($nivelusuario==10 || $nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) { ?><input type="checkbox" name="iusuariogusl1" checked><? } ?><? if($nivelusuario==10) { ?><select name="iusuariogusb1" class=textogeneralform><option value="" selected></option><option value="=">igual</option><option value="&lt;&gt;">diferente</option><option value="LIKE">parecido</option><option value="&lt;">menor que</option><option value="&gt;">mayor que</option><option value="&lt;=">menor o igual que</option><option value="&gt;=">mayor o igual que</option></select><select name="iusuariogusb2" onChange="if(iusuariogusb1.selectedIndex==0) iusuariogusb1.selectedIndex=1" class=textogeneralform><option value="0" selected></option><?  leecampos("usuarios","id","nombreusuario","",""," "); for ( $i=0; $i<=$registros-1; $i++) { echo("<option value=".$campos1[$i]); if($iusuariogus==$campos1[$i]) { echo(" selected"); } echo(">".$campos2[$i]."</option>"); } ?></select><? } ?></td></tr><? } ?>
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle">Reto</td><td valign="middle"><? if($nivelusuario==10 || $nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) { ?><input type="checkbox" name="iretgusl1" checked><? } ?><? if($nivelusuario==10) { ?><select name="iretgusb1" class=textogeneralform><option value="" selected></option><option value="=">igual</option><option value="&lt;&gt;">diferente</option><option value="LIKE">parecido</option><option value="&lt;">menor que</option><option value="&gt;">mayor que</option><option value="&lt;=">menor o igual que</option><option value="&gt;=">mayor o igual que</option></select><select name="iretgusb2" onChange="if(iretgusb1.selectedIndex==0) iretgusb1.selectedIndex=1" class=textogeneralform><option value="0" selected></option><?  leecampos("ret","id","nombreret","",""," "); for ( $i=0; $i<=$registros-1; $i++) { echo("<option value=".$campos1[$i]); if($iretgus==$campos1[$i]) { echo(" selected"); } echo(">".$campos2[$i]."</option>"); } ?></select><? } ?></td></tr><? } ?>
<? if($nivelusuario<>11 && $nivelusuario<>3 && $nivelusuario<>4) { ?><tr bgcolor="#<?=$vsitioscolor5?>" ><td valign="middle">Fecha</td><td valign="middle"><? if($nivelusuario==10 || $nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) { ?><input type="checkbox" name="fechagusl1"><? } ?><? if($nivelusuario==10) { ?><select name="fechagusb1" class=textogeneralform><option value="" selected></option><option value="=">igual</option><option value="&lt;&gt;">diferente</option><option value="LIKE">parecido</option><option value="&lt;">menor que</option><option value="&gt;">mayor que</option><option value="&lt;=">menor o igual que</option><option value="&gt;=">mayor o igual que</option></select><input type="text" name="fechagusb2" value="" size="50" onKeyUp="revisainput('fechagusb1','fechagusb2');" maxlength="50" class=textogeneralform><? } ?></td></tr><? } ?> 
    ...
    <tr> 
      ...
      <div align="right"><? if($ocultabotones<>1) { ?><input class="textogeneral" type="button" value="<?=$ib_busqueda?>" name=button1 onClick="return BusquedaNormal('gus.php?step=busqueda2&mensajemmx=<?=$mensajemmx?>&boletinelectronicommx=<?=$boletinelectronicommx?><?=$url_extra?>');">
<? if($nivelusuario==0 || $nivelusuario==1 || $nivelusuario==2) {?>
<input class="textogeneral" type="button" value="<?=$ib_exportar?>" name=button2 onClick="return BusquedaExcel('excel/excelgus.php?step=busqueda2<?=$url_extra?>');">
<?} ?><? } ?></div>
      </td>
    </tr>
