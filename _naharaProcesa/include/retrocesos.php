...
/*
	class retrocesos {
	    ...
	    {
	    	$this->iddon=$id;
			$this->cualtransaccion();
			$this->Init();
	    }

	    //regresa la url de refund
	    private function cualtransaccion(){


	    	
	    	$this->URL_refund="";
	    	$res=@mysqli_query($GLOBALS["enlaceDB"] ,"select urlrefund from transacciones where idon=".$this->iddon." and state='APPROVED'");
	    	if(mysqli_num_rows($res)>0){
	    		$datos=mysqli_fetch_object($res);
	    		$this->URL_refund=$datos->urlrefund;
	    	}
	    }
