...
<title><? echo("Categorías"); if(isset($titulobusqueda)) echo(". ".$titulobusqueda);?></title>
...
?>
  ...
  <tr>    
    ...
    <td height="30" valign="middle" align="left" style="white-space:nowrap"><? if($ocultabotones<>1) { ?><? $linkx3="";$linkx2="";$linkx1="";$linkx="";$idx3=0;$idx2=0;$idx1 =0;$idx=0;if($step=="modify"){$resultx = @mysqli_query($GLOBALS["enlaceDB"] ,"SELECT id,tipocat FROM cat where id=". $id);$rowx = mysqli_fetch_array($resultx);$linkx=" >> ".$rowx["tipocat"]." ".$rowx[""];$idx=$rowx[""];}echo("<a href=cat.php?step=1".$url_extra."><span class=titulo>Categorías</span></a>".$linkx3.$linkx2.$linkx1.$linkx);?><? } else { ?><? if(isset($titulobusqueda)) echo($titulobusqueda." ");?><? } ?></td>
	<td align="left" ><? if($ocultabotones<>1) { ?><? $botones=""; if($nivelusuario==0) $botones.="<td><a href=cat.php?step=busqueda3".$url_extra."><img src=recursos/botonlistar.gif border=\"0\" alt=\"Listar Categorías\"></a></td>";if($nivelusuario==0) $botones.="<td><a href=cat.php?step=busqueda".$url_extra."><img src=recursos/botonbuscar.gif border=\"0\" alt=\"Buscar Categorías\"></a></td>";if(($nivelusuario==0)) $botones.="<td><a href=\"cat.php?step=add".$url_extra."\"><img src=recursos/botonagregar.gif border=\"0\" alt=\"Agregar Categorías\"></a></td>"; if($_GET["edicioninterior"]<>1) echo("<table class=\"textogeneral\"><tr><td class=\"textogeneral\" align=\"right\">".$botones);echo("</tr></table>"); ?><? } else echo("<a href=\"javascript:self.parent.tb_remove();\"><img src=\"recursos/botoncerrar.gif\" border=\"0\"></a>"); ?></td>	
  </tr>
</table>
<? } 
	  ...
	  <table width="100%" class="titulointerior" cellpadding="0" cellspacing="0">
        <tr>
          <td valign=middle class="titulointerior"><?=$ib_busqueda?></td>
              <td class=textogeneral align="right"><? if($ocultabotones<>1) { ?> <?=$ib_ordenar?><select class="textogeneralform" name=sortfield><option value="nombrecat">Nombre</option><option value="i_nombrecat">Nombre (inglés)</option><option value="destacadacat">Destacada</option><option value="colorcat">Color</option><option value="imagencat">Imagen</option><option value="descripcioncat">Descripción</option><option value="i_descripcioncat">Descripción en inglés</option></select><select class="textogeneralform" name=ordenamiento><option value=DESC>DESC</OPTION><option value=ASC selected>ASC</OPTION></SELECT>
